...
# There may occur errors during rounding. This i caused by python's rounding-method.
def strToFloat(s: str):  # Takes string
    stringSplit: list[str] = s.split(".")
    if len(stringSplit) == 1:
        return int([0]) + 0.0  # "+0.0" converts given input to string
    return int(stringSplit[0]) + sum([int(stringSplit[1][_-1]) / 10**_ for _ in range(1, len(stringSplit[1])+1)]) # Takes sum of 10th, 100th and 1000th and so on.
...
class Vector:
    ...
    '''
    def __init__(self, pos: Point = None, end: Point = None, length: float = None, angle: float = None, colour: str = "red"):
        """
