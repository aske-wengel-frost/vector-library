...
class Point:
    ...
    Methods:
        ...
        __getitem__(self, key: int) -> float
            ...
            Index 0 corresponds to the x-coordinate, and index 1 corresponds to
        ...
        __str__(self) -> str
            Returns a string representation of the Point object in the format "
            (x; y)" with two decimal places.
...
class Vector:
    ...
    Attributes:
        ...
        angle (float): The angle (in degrees) between the vector and the posit-
        ive x-axis.
    ...
    Methods:
        __init__(self, pos: Point = None, end: Point = None, length: float = None,
            ...
            Create a Vector object using one of the following methods:
                1. Specify length and angle to calculate the endpoint from the
                starting point.
        ...
        __eq__(self, value: Vector) -> bool
            Returns True if the x and y values of both vectors are the same, ig-
            noring position.
    ...
    def render(self, p, ax):
        ...
        plt.text(
            self.pos[0] + self.x / 2,
            self.pos[1] + self.y / 2 + 0.3,
            color=self.color,
            s=f"r: {self.length:.2f}",
            rotation=self.angle,
